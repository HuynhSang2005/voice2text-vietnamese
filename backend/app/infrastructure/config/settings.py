...
class Settings(BaseSettings):
    ...
    MODEL_STORAGE_PATH: str = Field(
        default="models_storage", description="Base path for ML model storage"
    )
    ZIPFORMER_MODEL_PATH: str = Field(
        default="models_storage/zipformer/hynt-zipformer-30M-6000h",
        description="Path to Zipformer STT model directory",
    )
    VISOBERT_MODEL_PATH: str = Field(
        default="models_storage/visobert-hsd-span/onnx",
        description="Path to ViSoBERT-HSD-Span ONNX model directory",
    )

    # Content Moderation (ViSoBERT-HSD)
    ENABLE_CONTENT_MODERATION: bool = Field(
        default=True, description="Enable hate speech detection"
    )
    ...
    DATABASE_URL: str = Field(
        default="sqlite+aiosqlite:///database.db", description="SQLAlchemy database URL"
    )
    ...
    DATABASE_MAX_OVERFLOW: int = Field(
        default=10, ge=0, description="Max connections beyond pool size"
    )
    ...
    REDIS_DEFAULT_TTL: Optional[int] = Field(
        default=3600, ge=0, description="Default cache TTL in seconds"
    )
    REDIS_TTL: int = Field(
        default=3600,
        ge=0,
        description="Default cache TTL in seconds (deprecated, use REDIS_DEFAULT_TTL)",
    )
    REDIS_SERIALIZER: str = Field(
        default="pickle", description="Redis serializer: 'pickle' or 'json'"
    )
    ...
    DI_AUTO_WIRE: bool = Field(
        default=True, description="Automatically wire DI container"
    )
    DI_PACKAGES: List[str] = Field(
        default=["app.api", "app.application"], description="Packages to wire for DI"
    )
    ...
    def validate_redis_serializer(cls, v: str) -> str:
        """Validate Redis serializer format."""
        valid_serializers = ["pickle", "json"]
        v_lower = v.lower()
        if v_lower not in valid_serializers:
            raise ValueError(f"REDIS_SERIALIZER must be one of: {valid_serializers}")
        return v_lower
