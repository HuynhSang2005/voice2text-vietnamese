...
class TranscriptionRepositoryImpl(ITranscriptionRepository):
    ...
    async def find_all(
        ...
        try:
            ...
            if is_offensive is not None:
                if is_offensive:
                    ...
                    statement = statement.where(
                        ...
                        & (
                            (TranscriptionModel.moderation_label == "CLEAN")
                            | (TranscriptionModel.moderation_label.is_(None))
                        )
                    )
            ...
            # Order and paginate
            statement = (
                statement.order_by(TranscriptionModel.created_at.desc())
                .offset(skip)
                .limit(limit)
            )
    ...
    def _to_model(self, entity: Transcription) -> TranscriptionModel:
        ...
        return TranscriptionModel(
            ...
            detected_keywords=(
                entity.detected_keywords if entity.detected_keywords else None
            ),
        )
