...
class ModelManager:
    ...
    def start_model(self, model_name: str) -> None:
        ...
        if model_name not in self.VALID_MODELS:
            raise ValueError(
                f"Unknown model: {model_name}. Valid options: {self.VALID_MODELS}"
            )
    ...
    def get_queues(
        self, model_name: str
    ) -> Tuple[Optional[multiprocessing.Queue], Optional[multiprocessing.Queue]]:
    ...
    def start_span_detector(self, detector_name: str = "visobert-hsd-span") -> None:
        ...
        if detector_name not in self.VALID_SPAN_DETECTORS:
            raise ValueError(
                f"Unknown span detector: {detector_name}. Valid options: {self.VALID_SPAN_DETECTORS}"
            )

        if (
            self.current_span_detector == detector_name
            and self.span_detector_process is not None
        ):
        ...
        try:
            ...
            if not span_detector_class:
                raise ValueError(
                    f"No worker implementation for span detector: {detector_name}"
                )
    ...
    def stop_span_detector(self) -> None:
        ...
        if process.is_alive():
            logger.warning(
                f"Span detector {detector_name} did not stop gracefully, terminating"
            )
            process.terminate()
            process.join(timeout=5)
            ...
            if process.is_alive():
                logger.error(
                    f"Span detector {detector_name} still alive after terminate, killing"
                )
                process.kill()
