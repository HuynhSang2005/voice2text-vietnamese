...
class TranscriptionRequest(BaseModel):
    ...
    model: str = Field(
        default="zipformer", description="Model identifier for transcription"
    )
    sample_rate: int = Field(
        default=16000, ge=8000, le=48000, description="Audio sample rate in Hz"
    )
    enable_moderation: bool = Field(
        default=True, description="Enable content moderation for transcribed text"
    )
    session_id: Optional[str] = Field(
        default=None, description="Session ID for grouping related transcriptions"
    )
    language: str = Field(
        default="vi", description="Target language code (vi=Vietnamese, en=English)"
    )
    ...
    def validate_sample_rate(cls, v: int) -> int:
        ...
        if v not in standard_rates:
            raise ValueError(f"Sample rate must be one of {standard_rates}, got {v}")
        return v
...
class HistoryQueryRequest(BaseModel):
    ...
    search: Optional[str] = Field(
        default=None, max_length=200, description="Text search query"
    )
    model_filter: Optional[str] = Field(
        default=None, description="Filter by model identifier"
    )
    session_id: Optional[str] = Field(default=None, description="Filter by session ID")
    start_date: Optional[datetime] = Field(
        default=None, description="Filter records created after this date"
    )
    end_date: Optional[datetime] = Field(
        default=None, description="Filter records created before this date"
    )
    order_by: str = Field(default="created_at", description="Field to order results by")
    ...
    def validate_order_by(cls, v: str) -> str:
        ...
        if v not in allowed_fields:
            raise ValueError(f"order_by must be one of {allowed_fields}, got {v}")
        return v
...
class ModelSwitchRequest(BaseModel):
    ...
    force: bool = Field(default=False, description="Force switch even if model is busy")
    ...
    class Config:
        json_schema_extra = {"example": {"model_id": "zipformer", "force": False}}
...
class ModerationToggleRequest(BaseModel):
    ...
    enabled: bool = Field(..., description="Enable or disable content moderation")
...
class WebSocketConfigMessage(BaseModel):
    ...
    type: Literal["config"] = Field(
        default="config", description="Message type identifier"
    )
    model: str = Field(default="zipformer", description="Model identifier")
    sample_rate: int = Field(
        default=16000, ge=8000, le=48000, description="Audio sample rate in Hz"
    )
    moderation: bool = Field(default=True, description="Enable content moderation")
