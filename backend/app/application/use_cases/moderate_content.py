    ...
    def __init__(self, moderation_worker: IModerationWorker):
        ...
        if moderation_worker is None:
            raise ValidationException(
                field="moderation_worker", value=None, constraint="must not be None"
            )

        self._moderation_worker = moderation_worker

    async def execute(
        self, request: StandaloneModerateRequest
        ...
        if request is None:
            raise ValidationException(
                field="request", value=None, constraint="must not be None"
            )
        ...
        try:
            ...
            should_flag = result.is_harmful() and result.confidence >= request.threshold
...
class GetModerationStatusUseCase:
    ...
    async def execute(self) -> dict:
        ...
        if self._moderation_worker is None:
            return {"is_available": False, "is_ready": False, "model_version": None}
