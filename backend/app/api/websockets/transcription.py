...
from app.domain.exceptions import BusinessRuleViolationException as BusinessRuleException
    ...
    try:
                        ...
                        try:
                            ...
                            if msg_type == "config":
                                ...
                                config = TranscriptionRequest(
                                    ...
                                    enable_moderation=data.get("enable_moderation", config.enable_moderation),
                                    session_id=data.get("session_id", config.session_id),
                                    language=data.get("language", config.language)
                                )
                                logger.info(f"Updated config: model={config.model}, moderation={config.enable_moderation}")
        ...
        async def process_and_send():
            ...
            try:
                ...
                logger.info(f"Starting transcription: model={config.model}, moderation={config.enable_moderation}")
                ...
                async for transcription in use_case.execute(
                    ...
                    if ws_closed.is_set():
                        logger.debug("WebSocket closed, discarding transcription result")
                        continue
                    ...
                    result = {
                        ...
                        "is_final": transcription.is_final if hasattr(transcription, 'is_final') else True,
                    ...
                    try:
                        ...
                        logger.info(f"Sent transcription: '{transcription.content[:50]}...'")
        ...
        try:
            if not ws_closed.is_set():
                await websocket.close(code=status.WS_1000_NORMAL_CLOSURE, reason="Normal closure")
