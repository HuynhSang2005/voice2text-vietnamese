...
@router.get(
    "/api/v1/models", response_model=List[ModelInfo], summary="List available models"
)
...
@router.get(
    "/api/v1/history",
    response_model=List[TranscriptionLog],
    ...
    try:
        ...
        if not input_q or not output_q:
            await websocket.close(
                code=status.WS_1011_INTERNAL_ERROR, reason="Failed to start model"
            )
            return
        ...
        if moderation_enabled:
            try:
                ...
                span_detector_input_q, span_detector_output_q = (
                    manager.get_span_detector_queues()
                )
                if span_detector_input_q and span_detector_output_q:
                    logger.info(
                        "Content moderation enabled with visobert-hsd-span (unified)"
                    )
                else:
                    logger.warning(
                        "Failed to get span detector queues, moderation disabled"
                    )
                    moderation_enabled = False
            except Exception as e:
                logger.warning(
                    f"Failed to start span detector: {e}, moderation disabled"
                )
                moderation_enabled = False
                ...
                while True:
                    ...
                    if "bytes" in message:
                        ...
                        if audio_packet_count % 50 == 0:
                            logger.debug(
                                f"Received audio packet #{audio_packet_count}, size: {len(audio_data)} bytes"
                            )
                        ...
                        try:
                            ...
                            if msg_type == "config":
                                ...
                                if "moderation" in data:
                                    ...
                                    logger.info(
                                        f"Moderation toggled to: {moderation_enabled}"
                                    )
                                ...
                                if new_session_id:
                                    ...
                                    if drained_count > 0:
                                        logger.info(
                                            f"Drained {drained_count} stale transcription results from previous session"
                                        )
                                    ...
                                    if span_detector_output_q:
                                        ...
                                        if span_drained > 0:
                                            logger.info(
                                                f"Drained {span_drained} stale moderation results"
                                            )
                                ...
                                logger.info(
                                    "Received flush signal - forcing transcription of remaining buffer"
                                )
                ...
                while True:
                    ...
                    try:
                        ...
                        if not is_empty:
                            ...
                            if ws_closed.is_set():
                                logger.debug(
                                    "WebSocket already closed, discarding result"
                                )
                            ...
                            try:
                                ...
                                logger.info(
                                    f"Sent result #{result_count} to client: '{result.get('text', '')[:50]}...'"
                                )
                            ...
                            if should_moderate:
                                ...
                                try:
                                    await asyncio.to_thread(
                                        span_detector_input_q.put_nowait,
                                        moderation_request,
                                    )
                                    logger.info(
                                        f"Sent to moderation (final={is_final}): '{text_content[:40]}...'"
                                    )
                                except Exception as e:
                            ...
                            workflow_type = result.get(
                                "workflow_type", "streaming"
                            )  # Default to streaming for zipformer
                            ...
                            if receive_ended.is_set():
                                ...
                                if empty_checks >= max_empty_checks:
                                    logger.info(
                                        f"Receive ended and queue empty for {empty_checks * 50}ms, closing send_results"
                                    )
                                    break
                ...
                while True:
                    ...
                    try:
                        is_empty = await asyncio.to_thread(
                            lambda: span_detector_output_q.empty()
                        )
                        if not is_empty:
                            ...
                            if ws_closed.is_set():
                                logger.debug(
                                    "WebSocket closed, discarding moderation result"
                                )
                                continue
                            ...
                            detected_keywords = moderation_result.get(
                                "detected_keywords", []
                            )
                            ...
                            try:
                                ...
                                logger.info(
                                    f"Sent moderation: {label} ({confidence:.1%}) {flagged_str}{keywords_str}"
                                )
                                ...
                                logger.warning(
                                    f"Failed to send moderation result: {send_err}"
                                )
                                ws_closed.set()
                        else:
                            await asyncio.sleep(0.05)
                        ...
                        if receive_ended.is_set():
                            ...
                            is_still_empty = await asyncio.to_thread(
                                lambda: span_detector_output_q.empty()
                            )
                            if is_still_empty:
                                break
        ...
        moderation_task = (
            asyncio.create_task(send_moderation_results())
            if span_detector_output_q
            else None
        )
...
async def _save_transcription(
    ...
    async with async_session() as session:
        try:
            ...
            statement = select(TranscriptionLog).where(
                TranscriptionLog.session_id == session_id
            )
            ...
            if existing_log:
                ...
                if content:
                    if workflow_type == "streaming":
                        ...
                        logger.debug(
                            f"Replaced transcription for session {session_id}: '{content[:30]}...'"
                        )
                    else:
                        ...
                        if existing_log.content:
                            ...
                            existing_log.content = content
                        logger.debug(
                            f"Appended transcription to session {session_id}: '{content[:30]}...'"
                        )
                ...
                if detected_keywords is not None:
                    ...
                    merged_keywords = list(
                        dict.fromkeys(existing_keywords + detected_keywords)
                    )
...
@router.get(
    "/api/v1/models/status", response_model=ModelStatus, summary="Get model status"
)
...
@router.get(
    ...
    summary="Get content moderation status",
)
...
@router.post(
    "/api/v1/moderation/toggle",
    response_model=ModerationToggleResponse,
    summary="Toggle content moderation",
)
