...
class ContentModeration(BaseModel):
    ...
    label_id: int = Field(
        ge=0, le=2, description="Label ID: 0=CLEAN, 1=OFFENSIVE, 2=HATE"
    )
...
class ModerationConfig(BaseModel):
    ...
    default_enabled: bool = Field(
        description="Whether moderation is enabled by default"
    )
    confidence_threshold: float = Field(
        ge=0.0, le=1.0, description="Minimum confidence to consider a result valid"
    )
    on_final_only: bool = Field(
        description="Only run moderation on is_final=True results"
    )
...
class ModerationStatus(BaseModel):
    ...
    span_detector_active: bool = Field(
        default=False, description="Whether the span detector model is loaded and ready"
    )
