...
class SpanDetectorWorker(BaseWorker):
    ...
    MODERATION_LABEL_MAP = {0: "CLEAN", 1: "OFFENSIVE", 2: "HATE"}
    ...
    def __init__(
        self, input_queue, output_queue, model_name: str = "visobert-hsd-span"
    ):
    ...
    def load_model(self) -> None:
        ...
        if os.path.exists(int8_path) and os.listdir(int8_path):
            ...
            raise FileNotFoundError(
                ...
                "Please run 'python scripts/setup_hsd_span_model.py' to download and convert the model."
            )
    ...
    def _detect_spans(self, text: str, request_id: Optional[str] = None) -> Dict:
        ...
        model_spans = self._extract_spans(
            text, predictions, offset_mapping, attention_mask
        )
    ...
    def _extract_spans(
        ...
        for idx, (pred, offsets, mask) in enumerate(
    ...
    def _fallback_detect_spans(self, text: str) -> List[Dict[str, any]]:
        ...
        for bad_word in sorted_words:
            ...
            for match in re.finditer(pattern, text_lower):
                ...
                spans.append({"text": original_text, "start": start, "end": end})
    ...
    def _merge_spans(
        self, model_spans: List[Dict[str, any]], fallback_spans: List[Dict[str, any]]
    ) -> List[Dict[str, any]]:
        ...
        for fb_span in fallback_spans:
            ...
            if not overlapping_model_spans:
                ...
                fb_len = len(fb_range)
                all_model_shorter = all(
                    fb_len > m_len for _, _, m_len in overlapping_model_spans
                )
                ...
                if all_model_shorter:
                    ...
                    model_spans_to_keep = [
                        s
                        for i, s in enumerate(model_spans_to_keep)
