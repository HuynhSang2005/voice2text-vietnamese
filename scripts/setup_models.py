...
Models:
    ...
    2. ViSoBERT-HSD - Vietnamese Hate Speech Detection (content moderation)
...
Requirements:
    ...
    - ~500MB disk space total
      - Zipformer: ~200MB
      - ViSoBERT-HSD: ~100MB (INT8 quantized)
...
# ViSoBERT-HSD Model (HuggingFace)
VISOBERT_MODEL_NAME = "visolex/visobert-hsd"
VISOBERT_DIR = MODELS_DIR / "visobert-hsd"
...
def verify_visobert(model_path: Path, model_name: str) -> bool:
    """Verify that ViSoBERT model can run inference."""
    try:
        from optimum.onnxruntime import ORTModelForSequenceClassification
        from transformers import AutoTokenizer
        import torch
        ...
        for text in test_texts:
            ...
            print(f"       \"{text[:30]}...\" → {label_map[pred_id]} ({confidence:.0%})")
...
def setup_visobert(force: bool = False) -> bool:
    """
    Setup ViSoBERT-HSD model with ONNX conversion and INT8 quantization.
    
    Returns:
        True if successful, False otherwise
    """
    print_header("Setting up ViSoBERT-HSD (Content Moderation)")
    ...
    if int8_model.exists() and not force:
        ...
        print_success(f"ViSoBERT-HSD already set up ({size_mb:.1f} MB)")
    ...
    if success:
        print_success("ViSoBERT-HSD setup complete!")
    else:
        print_error("ViSoBERT-HSD setup had errors.")
